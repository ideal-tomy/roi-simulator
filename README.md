...
```jsonc
{
  ...
  // デモ直リンク時のヒーロー（任意。あると業種カテゴリ文言より優先）
  "hero": {
    "eyebrow": "飲食・シフト管理",
    "title": "希望を集めて、埋めて、配る。\nその時間が消えたら？",
    "lead": "店長のシフト業務を下書きまで自動化します。"
  },
  "questions": [
    {
      "id": "staff",
...
`?kit=` 付きで開くと見積質問へ自動スクロールする。`hero` がある kit は業種汎用ヒーローの代わりにその文言を先頭表示する。

**見積もりを閉じる**と `kit` だけ外れ、業界・カテゴリを選べる**閲覧モード**に戻る（`industry` / `cat` / `from` / `brand` は維持）。デモ直リンク入場後も、閉じたあとは業界ピッカーが表示される。入口／出口の正本は [`docs/demo-roi-integration-playbook.md`](./docs/demo-roi-integration-playbook.md)。
